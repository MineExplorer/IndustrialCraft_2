...
for (var i_1 = 1; i_1 < 7; i_1++) {
    newGuiMatronObject.elements["slotWeedEx" + i_1] = { type: "slot", x: 441 + 60 * i_1, y: 155, isValid: isWeedEx };
}
for (var i_2 = 1; i_2 < 7; i_2++) {
    newGuiMatronObject.elements["slotFertilizer" + i_2] = { type: "slot", x: 441 + 60 * i_2, y: 75, isValid: isFertilizer };
}
...
MachineRegistry.registerElectricMachine(BlockID.cropMatron, {
    ...
    getSlot: function (type) {
        for (var i_3 = 0; i_3 < 7; i_3++) {
            var slot = this.container.getSlot(type + i_3);
            if (slot.id)
                return slot;
        }
        return null;
